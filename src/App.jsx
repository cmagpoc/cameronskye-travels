...
Write like a friend who has actually been there. Inspiring but practical.`;
  ...
  function stripEmoji(str) {
    return str.replace(/[\u{1F300}-\u{1FFFF}\u{2600}-\u{26FF}\u{2700}-\u{27BF}\u{FE00}-\u{FE0F}\u{1F900}-\u{1F9FF}]/gu, "").replace(/[^\x00-\x7F]/g, "").trim();
  }
  ...
  function downloadPDF() {
    ...
    const safeTitle = stripEmoji(itineraryTitle || "My Europe Itinerary");
    const safeText = stripEmoji(itineraryText);
    const safeDestination = stripEmoji(form.destination);
    const safeDuration = stripEmoji(form.duration);
    const safeStyle = stripEmoji(form.style);
    const safeBudget = stripEmoji(form.budget);
    ...
    doc.text(`${safeDestination} · ${safeDuration} · ${safeStyle} · ${safeBudget}`, margin, 31);
    ...
    lines.forEach(line => {
      ...
      if (line.match(/^Day \d+/i)) {
        ...
        doc.setFontSize(12);
        y += 3;
      } else if (line.match(/^(Morning|Afternoon|Evening|Food|Photography|Tip|Budget|PTO)/i)) {
        doc.setFont("helvetica", "bold");
        doc.setTextColor(28, 43, 32);
        doc.setFontSize(11);
      } else {
    ...
    const filename = `${safeDestination.toLowerCase().replace(/\s+/g, "-")}-${safeDuration.replace(/\s+/g, "")}-itinerary.pdf`;
    doc.save(filename);
  }
